...
public class ServerUtility {
    ...
    public static int executeCommand(Socket socket, String[] args) {
        ...
        switch (command) {
            case "help":
                return ServerUtility.sendHelp(socket);
            case "tz":
                return ServerUtility.sendTimeZone(socket, args[1]);
            case "tn":
                if (args.length >= 4) {
                    int from = Integer.parseInt(args[2]);
                    int to = Integer.parseInt(args[3]);
                    return ServerUtility.sendTranslatedNumber(socket, args[1], from, to);
                }
                break;
            case "exit":
                return -1;
        }
        SocketUtility.sendMessage(socket, "Unknown command! Type 'help' to get help");
        return 0;
    }

    // Function that sends a predefined text to the client that shows him the commands
    private static int sendHelp(Socket socket) {
    ...
    private static int sendTimeZone(Socket socket, String timeZone) {
        ...
        return SocketUtility.sendMessage(socket, df.format(date));
    }
